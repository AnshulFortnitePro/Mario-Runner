...
function setup() {
 ...
 // createCanvas(600, 200);
  mario = createSprite(50,height-180,20,50);
  mario.addAnimation("running", mario_running);
  mario.scale = 0.5;
  
  ground = createSprite(0,height-100,1200,10);
  ground.x = ground.width /2;
  ground.velocityX = -(6 + 3*score/100);
  
  gameOver = createSprite(300,height- 190);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyDown("space") && mario.y >= height-140) {
      mario.velocityY = -12;
    }
...
function spawnCoin() {
  ...
  if (frameCount % 60 === 0) {
    var coin = createSprite(600,height-120,40,10);
    coin.y = Math.round(random(height-170,height-120));
